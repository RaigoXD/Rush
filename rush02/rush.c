...
void rush02(int x, int y) // Funcion rush00 sera la funcion intermedia para que asi de esta manera podamos llamar a rush con solo dos parametros
{
		...
		if(contadorx <= x)  //Si el contadorX es menor igual a x
		{
			if (contadory == 1 || contadory == y) //Si se esta en la primera o ultima fila se hara lo siguiente
	    	{
	      		if(contadory == 1){ //Si estamos en la primera fila 
					if(contadorx == 1 || contadorx == x) //Si estamos en la esquina superior izquierda v en la esquina superior derecha
		  				printf ("A"); 
					else
		  				printf ("B");   //si ninguna de las anteriores es correcta estamos entre medias
					
				}else
				{
					if(contadorx == 1 || contadorx == x)  //si estamos en la ultima fila y estamos en la esquina inferior izquierda
		  				printf ("C");  
	      			else
